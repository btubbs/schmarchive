string myurl;
string base = "http://schmarchive.btubbs.com/";

integer SAY_URL = -85431;
integer HTTP_RESPONSE = -85432;

integer INV_REQUEST = -85433;
integer INFO_REQUEST = -85434;
...
// string SECRET = "";
string SECRET = "ek4dBCGEVTqiYdHEIryMr7Va+Y98eSjC";
...
BuildRoutes() {
    ...
    for (n = 0; n < script_count; n++) {
        ...
        //valid plugins have 3 parts, the first named .schmottp, and the last a non-0 integer
        if (llGetListLength(parts) == 3 && 
            llList2String(parts, 0) == ".schmottp" && 
...
default
{
    state_entry() {
        if (SECRET == "") {
            ...
            list parts = llParseStringKeepNulls(myname, [":"], []);
    ...
    touch_start(integer num) {
        ...
        llInstantMessage(llDetectedKey(0), BuildURL(id));
    }
    ...
    http_request(key id, string method, string body) {
        if (method == "URL_REQUEST_GRANTED") {
            ...
            if (CheckToken(av, tok)) {
                ...
                //see if this top level path corresponds to a channel (integer) in ROUTES.
                //if so, send link message on that channel
                string root = llList2String(pathparts, 0);
                integer index = llListFindList(routes, [root]);
                
                if (index != -1) {
                    //we have a valid route for this request.  So let's send it out
                    //stick the query string in the str field
